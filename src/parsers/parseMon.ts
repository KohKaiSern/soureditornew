...
export function parsePartyMon(file: Uint8Array, address: number): Omit<PartyMon, 'nickname' | 'OTNickname'> {
  return {
...
const moveset = (file: Uint8Array, address: number): string[] =>
  Array.from({ length: 4 }, (_, i) =>
    moves.find(m => m.index === file[address + i])!.name
  );
...
const statExps = (file: Uint8Array, address: number): number[] =>
  Array.from({ length: 5 }, (_, i) =>
    (file[address + i * 2] << 8) | file[address + i * 2 + 1]
  )

const dvs = (file: Uint8Array, address: number): number[] =>
  Array.from({ length: 4 }, (_, i) =>
    (file[address + Math.floor(i / 2)] >> (4 * (1 - i % 2))) & 0xF
  )
...
const pokerus = (file: Uint8Array, address: number): 'NONE' | {
  strain: number;
...
const status = (file: Uint8Array, address: number): {
  ...
  const name = ['SLEEP', 'SLEEP', 'SLEEP', 'POISON', 'BURN', 'FREEZE', 'PARALYSIS', 'BADLY POISONED']
...
const stats = (file: Uint8Array, address: number): number[] =>
  Array.from({ length: 6 }, (_, i) =>
    file[address + 2 * i] << 8 | file[address + 2 * i + 1]
  )
