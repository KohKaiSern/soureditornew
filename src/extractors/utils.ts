...
export async function writeJSON(name: string, obj: any): Promise<void> {
...
export async function applyPalette(
  ...
  const metadata = await sharp(
    import.meta.dirname + '/../../sourcrystal/' + inputPath
  ).metadata();
...
export async function createGIF(
  ...
  const animContent = (await readFile(
    import.meta.dirname + '/../../sourcrystal/' + animPath,
    'utf-8'
  ))
    .split('\n')
    .map((l) => l.trim());
